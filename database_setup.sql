-- =====================================================
-- INTRANET VOLLSTÄNDIGE DATENBANK-SETUP
-- =====================================================
-- Diese Datei enthält alle SQL-Skripte für das Intranet
-- Führen Sie dieses Skript komplett in Ihrem Neon SQL Editor aus
-- =====================================================
-- Erstellt: 2025
...
-- ==============================================
-- TRIGGER-FUNKTION (muss zuerst erstellt werden)
-- ==============================================
...
CREATE TABLE IF NOT EXISTS completed_trainings (
  ...
  participant_surname VARCHAR(255),
...
-- =====================================================
-- TEIL 2: BESPRECHUNGEN UND BESPRECHUNGSPUNKTE
-- =====================================================

-- Besprechungen Tabelle
CREATE TABLE IF NOT EXISTS besprechungen (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  ...
  ort VARCHAR(255),
  teilnehmer TEXT[], -- Array von Teilnehmernamen
  protokoll TEXT,
  status VARCHAR(50) DEFAULT 'geplant' CHECK (status IN ('geplant', 'durchgefuehrt', 'verschoben', 'abgesagt')),
  erstellt_von VARCHAR(255) NOT NULL,
  erstellt_am TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  aktualisiert_am TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Besprechungspunkte Tabelle
CREATE TABLE IF NOT EXISTS besprechungspunkte (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  besprechung_id UUID REFERENCES besprechungen(id) ON DELETE CASCADE,
  titel VARCHAR(255) NOT NULL,
  beschreibung TEXT,
  verantwortlich VARCHAR(255),
  frist VARCHAR(100),
  prioritaet VARCHAR(50) CHECK (prioritaet IN ('Niedrig', 'Mittel', 'Hoch')),
  status VARCHAR(50) DEFAULT 'offen' CHECK (status IN ('offen', 'in_arbeit', 'erledigt')),
  notizen TEXT,
...
-- Erledigungsvermerke Tabelle
CREATE TABLE IF NOT EXISTS erledigungsvermerke (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  besprechungspunkt_id UUID REFERENCES besprechungspunkte(id) ON DELETE CASCADE,
  text TEXT NOT NULL,
  erstellt_von VARCHAR(255) NOT NULL,
  erstellt_am TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS jour_fixe_entries (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  bereich VARCHAR(100) NOT NULL,
  kategorie VARCHAR(100) NOT NULL,
  vereinbart_am VARCHAR(100) NOT NULL,
  aufgabenfeld TEXT NOT NULL,
  klaerung TEXT,
  verantwortlich VARCHAR(255) NOT NULL,
  beteiligt VARCHAR(255),
  termin_soll VARCHAR(100),
  abschluss_termin VARCHAR(100),
  prioritaet VARCHAR(50) DEFAULT 'Mittel' CHECK (prioritaet IN ('Niedrig', 'Mittel', 'Hoch')),
  status VARCHAR(50) DEFAULT 'warte' CHECK (status IN ('warte', 'in Arbeit', 'erledigt')),
  pdf_url TEXT,
  pdf_name VARCHAR(255),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
-- Insert existing users with admin rights
INSERT INTO users (username, password, display_name, is_admin, is_active, role) VALUES
  ('Christof Drost', '12345', 'Christof Drost', true, true, 'Admin'),
  ('Kirstin', 'kirstin123', 'Kirstin Kreusch', true, true, 'Admin'),
  ('Julia', 'julia112', 'Julia Wodonis', false, true, 'Benutzer'),
  ('Lisa', 'lisa331', 'Lisa Schnagl', false, true, 'Benutzer'),
  ('Jonas', 'Jonas554', 'Jonas Jooss', false, true, 'Benutzer'),
  ('Dennis', 'Dennis812', 'Dennis Wilkens', false, true, 'Benutzer'),
  ('Lea', 'lea331', 'Lea Hofmann', false, true, 'Benutzer'),
  ('laola', 'laola123', 'Team LAOLA', false, true, 'Benutzer'),
  ('staho', 'staho123', 'Verwaltung Stadtholding Landau', false, true, 'Benutzer')
...
-- =====================================================
-- TEIL 6: INDEXES FOR PERFORMANCE
-- =====================================================
...
-- Besprechungen Indexes
CREATE INDEX IF NOT EXISTS idx_besprechungen_bereich ON besprechungen(bereich);
CREATE INDEX IF NOT EXISTS idx_besprechungen_status ON besprechungen(status);
CREATE INDEX IF NOT EXISTS idx_besprechungen_datum ON besprechungen(datum);
CREATE INDEX IF NOT EXISTS idx_besprechungspunkte_besprechung_id ON besprechungspunkte(besprechung_id);
...
-- Jour-fixe Indexes
CREATE INDEX IF NOT EXISTS idx_jour_fixe_bereich ON jour_fixe_entries(bereich);
CREATE INDEX IF NOT EXISTS idx_jour_fixe_kategorie ON jour_fixe_entries(kategorie);
CREATE INDEX IF NOT EXISTS idx_jour_fixe_status ON jour_fixe_entries(status);
CREATE INDEX IF NOT EXISTS idx_jour_fixe_verantwortlich ON jour_fixe_entries(verantwortlich);

-- =====================================================
-- TEIL 7: TRIGGERS FOR AUTO-UPDATING updated_at
-- =====================================================
...
-- Apply to besprechungen
DROP TRIGGER IF EXISTS update_besprechungen_updated_at ON besprechungen;
CREATE TRIGGER update_besprechungen_updated_at
  BEFORE UPDATE ON besprechungen
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- Apply to besprechungspunkte
...
-- Apply to jour_fixe_entries
DROP TRIGGER IF EXISTS update_jour_fixe_updated_at ON jour_fixe_entries;
CREATE TRIGGER update_jour_fixe_updated_at
  BEFORE UPDATE ON jour_fixe_entries
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- =====================================================
-- TEIL 8: QUIZ-DATEN EINFÜGEN
-- =====================================================
...
-- =====================================================
-- TEIL 9: KOMMENTARE FÜR DOKUMENTATION
-- =====================================================

COMMENT ON TABLE besprechungen IS 'Speichert Besprechungen mit Metadaten';
COMMENT ON TABLE besprechungspunkte IS 'Speichert einzelne Punkte einer Besprechung';
COMMENT ON TABLE erledigungsvermerke IS 'Speichert Erledigungsvermerke zu Besprechungspunkten';
COMMENT ON TABLE jour_fixe_entries IS 'Speichert Jour-fixe Einträge aus der Aufgaben-Seite';
...
BEGIN
  ...
  RAISE NOTICE '==============================================';
  RAISE NOTICE '✅ INTRANET DATENBANK-SETUP ABGESCHLOSSEN!';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '   1. Erstellen Sie eine .env.local Datei';
  RAISE NOTICE '   2. Fügen Sie DATABASE_URL hinzu';
