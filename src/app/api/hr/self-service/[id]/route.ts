...
const ALLOWED_FIELDS = ['employee_name', 'request_type', 'details', 'status']
...
export async function PATCH(
  ...
  try {
    ...
    const result = await query(
      `UPDATE hr_self_service_requests
       SET ${updates.join(', ')}
    ...
    console.error('Failed to update self-service request:', error)
    return NextResponse.json({ error: 'Failed to update self-service request' }, { status: 500 })
  }
}
