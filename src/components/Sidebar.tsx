...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { href: '/beschwerdemanagement', label: 'Beschwerden', icon: '⚠️' },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full ${isCollapsed ? 'w-20' : 'w-64'} bg-blue-900 lg:bg-blue-900/20 backdrop-blur-xl border-r border-blue-200/30 shadow-2xl z-50 transform transition-all duration-300 flex flex-col ${
      ...
      <div className="flex-shrink-0 p-4 border-b border-blue-200/30">
        <div className="flex items-center justify-center relative">
          <img 
            ...
            className={`${isCollapsed ? 'h-10' : 'h-24'} w-auto object-contain rounded-xl transition-all duration-300`}
          />
          <button
            type="button"
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="hidden lg:flex absolute -right-2 -bottom-2 h-8 w-8 items-center justify-center rounded-full bg-blue-700 text-white shadow-md hover:bg-blue-600 transition-colors"
      ...
      {/* User Info */}
      <div className="flex-shrink-0 p-4 border-b border-blue-200/30">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} p-3 bg-blue-800/30 backdrop-blur-sm rounded-xl border border-blue-300/30`}>
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center ${isCollapsed ? 'justify-center px-3' : 'space-x-3 px-4'} py-3 rounded-xl transition-all duration-200 relative ${
                    isActive
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`flex items-center ${isCollapsed ? 'justify-center px-3' : 'space-x-2 px-4'} py-2.5 rounded-xl transition-all duration-200 ${
                        isActive
      ...
      <div className="flex-shrink-0 p-4 border-t border-blue-200/30">
        <button
          onClick={logout}
          className={`w-full flex items-center justify-center ${isCollapsed ? '' : 'space-x-2'} px-4 py-3 text-white hover:text-white hover:bg-red-600/40 rounded-xl transition-all duration-200 backdrop-blur-sm border border-blue-300/30 hover:border-red-400/50`}
        >
          <span>🚪</span>
